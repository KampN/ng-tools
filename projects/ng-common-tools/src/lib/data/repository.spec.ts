...
describe('Data : Repository', () => {
    ...
    it('should verify if an item is outdated by its id', () => {
        const beforeNow = DateTime.now().minus({days: 1}).toUnixInteger();
        const afterNow = DateTime.now().plus({days: 1}).toUnixInteger();
        const items: any[] = [
    ...
    describe('refresh', () => {
        ...
        it('should refresh an all the queried items', inject(
            [HttpTestingController, HttpClient],
            (httpMock: HttpTestingController, http: HttpClient) => {
                const expiryDate = DateTime.now().toUnixInteger() + 8600;
    ...
    it('should mark the given ids as outdated using matching fn', () => {
        ...
        repository.markMatchingItemsAsOutdated((item) => [2].includes(item.id));
