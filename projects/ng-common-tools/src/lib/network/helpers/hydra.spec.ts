...
describe('Network : Hydra', () => {
    ...
    describe('id', () => {
        it('should return an hydra id when passing Hydra object or HydraId', () => {
            ...
            expect(Hydra.id(item['@id'])).toBeDefined();
        });
    ...
    describe('normalizeResponse', () => {
        it('should normalize an hydra collection', () => {
            const response: HydraCollection<DummyObject> = HydraFactory.generateCollection(dummyFactory.sperm(4), 10);
            ...
            expect(parsed.metadata).toEqual(expect.objectContaining(response['hydra:view']));
        });

        it('should normalize an hydra single result', () => {
            const response: HydraItem<DummyObject> = HydraFactory.generateItem(dummyFactory.seed({id: 1}), 'dummy');
